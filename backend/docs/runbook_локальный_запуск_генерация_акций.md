...
- Backend поддерживает `AI_PROVIDER=stub|gemini|groq` (`stub` — default).
- Для реального AI нужно задать `AI_PROVIDER=gemini` и `GEMINI_API_KEY`, либо `AI_PROVIDER=groq` и `GROQ_API_KEY`.
...
- Если `AI_PROVIDER=gemini` или `AI_PROVIDER=groq` и ключ не задан/невалиден, AI endpoints возвращают ошибку (fallback на frontend отключен)
...
## Prompt contracts (LLM, strict JSON)

Backend ожидает строгий JSON без markdown/code fences:

- `POST /ai/themes` -> `{ \"themes\": [{ \"value\": \"slug\", \"label\": \"...\" }] }` (ровно 1 тема)
